...
public:
  ...
  void await() ABSL_LOCKS_EXCLUDED(mtx_);

  void countdown() ABSL_LOCKS_EXCLUDED(mtx_);

  void increaseCount() ABSL_LOCKS_EXCLUDED(mtx_);

private:
  int32_t count_ ABSL_GUARDED_BY(mtx_);
